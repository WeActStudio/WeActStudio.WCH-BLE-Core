...
void Broadcaster_Init()
{
    ...
    tmos_start_task(Broadcaster_TaskID, SBP_ADV_UPDATE, MS1_TO_SYSTEM_TIME(1000));
}
...
uint16_t Broadcaster_ProcessEvent(uint8_t task_id, uint16_t events)
{
    ...
    if (events & SBP_ADV_UPDATE)
    {
        ...
        temp = adc_to_temperature_celsius(HAL_GetInterTempValue()) - 600;
        printf("temp: %d\r\n", temp);
        ...
        // Update advert
        GAP_UpdateAdvertisingData(Broadcaster_TaskID, TRUE, sizeof(advertData_update), advertData_update);
